import ProgrammesGrid from "../Components/Stages/Stages";

const Ug = () =>{
    return(
        <div class="container syllabus-container">
            <ProgrammesGrid
            heading= "BSc Courses"
            programmes= {[
                "BSc Hons Anthropology",
                "BSc Hons Biochemistry",
                "BSc Hons Biological Science",
                "BSc Hons Biomedical Science",
                "BSc Hons Botany",
                "BSc Hons Chemistry",
                "BSc Hons Computer Science",
                "BSc Hons Electronics",
                "BSc Hons Forensic Scie",
                "BSc Hons Instrumentation",
                "BSc Hons Maths",
                "BSc Hons Microbiology",
                "Bsc Hons Occupational Therapy",
                "BSc Hons Physics",
                "BSc Hons Statistics",
                "BSc Hons Zoology",
                "Bsc Analytical Chemistry",
                "Bsc Applied Life Sciences with Agro-Chemical & Pest Management",
                "Bsc Applied Physical Sciences (Industrial Chemistry)",
                "Bsc Fisheries Science",
                "Bsc Life Sciences",
                "BSc Mathematical Science",
                "BSc Physical Science with Computer Science",
                "BSc Physical Education, Health Education & Sports",
                "BSc Sericulture"
            ]}
            />
            <ProgrammesGrid
            heading="BA Courses"
            programmes= {[
				"B.A. Hons Mathematics",
				"B.A. Hons Economics",
				"B.A. Hons Business Economics",
				"B.A. Hons History",
				"B.A. Hons Geography",
				"B.A. Hons Political Science",
				"B.A. Hons Music (Hindustani)",
				"B.A. Hons Music (Karnatak)",
				"B.A. Hons Music Percussion (Tabla/Pakhawaj)",
				"B.A. Hons Philosophy",
				"B.A. Hons Applied Psychology",
				"B.A. Hons Psychology",
				"B.A. Hons Social Work",
				"B.A. Hons Sociology",
				"B.A. Hons Music",
				"B.A. Statistics",
				"B.A. Persian",
				"B.A. Philosophy",
				"B.A. Vocational Studies",
				"B.A. Office Administration & Secretarial Practice",
				"B.A. Fine Arts (BFA)",
				"B.A. Geography",
				"B.Ed. Education",
				"B.Ed. Home Science",
				"B.Ed. Mental Retardation Special Education ",
				"B.El.Ed. Elementary Education",
				"B.P.Ed.	Physical Education",
				"Bachelor Library and Information Science (B.Lib.I.Sc.)",
				"Bachelor Physical Education and Sports (B.P.E.S)",
				"Bachelor Tourism Management (B.T.M.)",
				"B.A. Hons Journalism",
				"B.A. Hons Hindi Journalism And Mass Communication",
				"B.A. Hons Patrakarita Evam Jansanchar",
				"B.A. Hons Multi Media & Mass Communication",
				"B.A. Mass Communication",
				"B.A. Human Resource Management",
				"B.A. Management & Marketing of Insurance",
				"B.A. Marketing Management & Retail Business",
				"B.A. Materials Management"
            ]}
            />
            <ProgrammesGrid 
            heading="BA Courses (Language)"
            programmes= {[
				"B.A. Hons Sanskrit",
				"B.A. Hons Hindi",
				"B.A. Hons Bengali",
				"B.A. Hons Punjabi",
				"B.A. Hons English",
				"B.A. Hons Arabic",
				"B.A. Hons Urdu",
				"B.A. Hons Spanish",
				"B.A. Hons Zongha",
				"B.A. Hons French",
				"B.A. Hons German",
				"B.A. Hons Italian",
				"B.A. Hons Persian",
				"B.A. Hindi",
				"B.A. Functional Hindi",
				"B.A. Arabic",
				"B.A. Studies",
				"B.A. Education",
				"B.A. Psychology",
				"B.A. Applications"
            ]}
            />
            <ProgrammesGrid 
            heading="Commerce & Management"
            programmes= {[
				"B.B.A. Business Administratio",
				"B.F.I.A. Financial and Investment Analysi",
				"B.Comm Commerc",
				"B.Comm Hons Commerc",
				"B.B.S. Business Studie",
				"B.M.S. Management Studies"
            ]}
            />
        </div>
    );
}

export default Ug;